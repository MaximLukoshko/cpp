...
double skalyar(double** array1, double* array2, int number_line, int number_col,
		int number) { //Скалярное произведение
	double sum = 0;
...
void CMatrix::Check_decision(double &d) {
	...
	int k = 0;
	fluct[k] = skalyar(m_Matrix, m_solution, k, 0, m_num_lines)
			- m_Matrix[k][m_num_columns - 1];
